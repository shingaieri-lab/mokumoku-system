// Google OAuth / Gmail ユーティリティ
// トークン取得・検証・メール下書き送信の共通処理

// OAuth コールバックエラーのハンドリング
export function handleOAuthCallbackError(resp, rej) {
  if (resp.error === 'redirect_uri_mismatch' || resp.error === 'invalid_client') {
    rej(new Error(
      `OAuth設定エラー: Google Cloud ConsoleのOAuthクライアントの「承認済みJavaScriptオリジン」に ` +
      `"${window.location.origin}" を追加してください。`
    ));
  } else {
...
export function handleOAuthPopupError(err, rej) {
  if (err.type === 'popup_blocked_by_browser') {
    ...
    rej(new Error(
      `OAuth設定エラー: Google Cloud ConsoleのOAuthクライアントの「承認済みJavaScriptオリジン」に ` +
      `"${window.location.origin}" が登録されているか確認してください。`
    ));
  }
}
...
export const postGmailDraft = async (token, raw) => {
  ...
  if (!res.ok) {
    ...
    if (err.error?.code === 401 || err.error?.status === 'UNAUTHENTICATED') {
      throw Object.assign(
        new Error('認証の期限が切れました。再度お試しください。'),
