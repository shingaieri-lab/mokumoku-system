...
export function GmailWizard({ go, dismiss, onUpdateProfile, currentUser, aiConfig, onSave }) {
    ...
    <WizardOverlay onDismiss={dismiss}>
      <div style={WIZARD_CARD}>
        ...
        <div style={{ display: "flex", gap: 8, justifyContent: "space-between" }}>
          ...
          <button onClick={() => setStep(3)} style={WIZARD_BTN_P}>完了 → 次へ</button>
        </div>
      </div>
    </WizardOverlay>
  );
    ...
    <WizardOverlay onDismiss={dismiss}>
      <div style={WIZARD_CARD}>
        ...
        <div style={{ marginBottom: 8 }}>
          ...
          <input value={gmailClientId} onChange={e => setGmailClientId(e.target.value)} placeholder="xxxxxxxxxx.apps.googleusercontent.com" style={WIZARD_INP} />
        </div>
        <div style={{ display: "flex", gap: 8, justifyContent: "space-between", marginTop: 20 }}>
          <button onClick={() => setStep(2)} style={WIZARD_BTN_S}>← 戻る</button>
          <button onClick={() => setStep(4)} disabled={!gmailClientId.trim()} style={{ ...WIZARD_BTN_P, opacity: gmailClientId.trim() ? 1 : 0.5 }}>次へ →</button>
        </div>
      </div>
    </WizardOverlay>
  );
    ...
    <WizardOverlay onDismiss={dismiss}>
      <div style={WIZARD_CARD}>
        ...
        <div style={{ display: "flex", gap: 8, justifyContent: "space-between" }}>
          ...
          <button onClick={() => { onUpdateProfile({ ...currentUser, gmailClientId: gmailClientId.trim() }); if (onSave) onSave({ ...(aiConfig||{}), gmailClientId: gmailClientId.trim() }); go("overview"); }} style={WIZARD_BTN_P}>保存して完了</button>
        </div>
      </div>
    </WizardOverlay>
  );
