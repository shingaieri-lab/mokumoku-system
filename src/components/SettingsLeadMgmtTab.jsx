...
import { DEFAULT_SOURCES, DEFAULT_STATUSES_WITH_COLORS, LEAD_SOURCE_ICONS, PALETTE } from '../lib/constants.js';
...
export function SettingsLeadMgmtTab({ master, onSave, onLeadsChange }) {
  ...
  const addStatus = () => {
    ...
    onSave({ ...master, statuses: [...getStatusData(), { label:l, color:newStatusForm.color }] });
    setNewStatusForm({ label:"", color:"#0ea5e9" });
  };
  const removeStatus = (idx) => {
    if (!window.confirm("削除しますか？")) return;
    onSave({ ...master, statuses: getStatusData().filter((_,i)=>i!==idx) });
  };
  ...
  const saveStatus = () => {
    ...
    onSave({ ...master, statuses: getStatusData().map((s,i)=>i===editStatusIdx?{...s,...editStatusForm,label:l}:s) });
  ...
  const moveStatus = (idx, dir) => {
    ...
    else return; onSave({ ...master, statuses: data });
  };
  ...
  const addSource = () => {
    ...
    onSave({ ...master, sources: [...currentSources, {label: s, icon: newSourceIcon}] });
    setNewSource(""); setNewSourceIcon("home");
  };
  const removeSource = (s) => {
    if ((master.sources||DEFAULT_SOURCES).length <= 1) { alert("最低1つ必要です"); return; }
    onSave({ ...master, sources: (master.sources||DEFAULT_SOURCES).filter(x => (typeof x === "string" ? x : x.label) !== s) });
  };
  ...
  const saveSource = async () => {
    ...
    onSave({ ...master, sources: newSources });
    if (newVal !== oldVal) {
