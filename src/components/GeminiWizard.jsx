...
export function GeminiWizard({ go, dismiss, onUpdateProfile, currentUser }) {
      ...
      <div style={WIZARD_CARD}>
        ...
        <div style={{ marginBottom: 8 }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <input value={geminiKey} onChange={e => { setGeminiKey(e.target.value); setGeminiTest(null); }} placeholder="AIzaSy..." style={{ ...WIZARD_INP, flex: 1 }} />
            <button onClick={() => testGemini(geminiKey.trim())} disabled={!geminiKey.trim() || geminiTest === "testing"}
              style={{ ...WIZARD_BTN_P, padding: "9px 12px", fontSize: 11, flexShrink: 0, opacity: (!geminiKey.trim() || geminiTest === "testing") ? 0.5 : 1 }}>
        ...
        <div style={{ display: "flex", gap: 8, justifyContent: "space-between", marginTop: 20 }}>
          <button onClick={() => { setStep(0); setGeminiTest(null); }} style={WIZARD_BTN_S}>← 戻る</button>
          <button onClick={() => setStep(2)} disabled={!geminiKey.trim()} style={{ ...WIZARD_BTN_P, opacity: geminiKey.trim() ? 1 : 0.5 }}>次へ →</button>
        </div>
      </div>
    </WizardOverlay>
  );
    ...
    <WizardOverlay onDismiss={dismiss}>
      <div style={WIZARD_CARD}>
        ...
        <div style={{ display: "flex", gap: 8, justifyContent: "space-between" }}>
          ...
          <button onClick={() => { onUpdateProfile({ ...currentUser, geminiKey: geminiKey.trim() }); go("overview"); }} style={WIZARD_BTN_P}>保存して完了</button>
        </div>
      </div>
    </WizardOverlay>
  );
