...
import { CheckCircleIcon, AlertIcon, RocketIcon, UserIcon, KeyIcon, ClipboardIcon, SendIcon, LightbulbIcon } from '../ui/Icons.jsx';
    ...
    <div>
      <div>
        ...
        <div style={{background: allOk ? "#f0fdf4" : "#fffbeb", border:`1px solid ${allOk?"#86efac":"#fde68a"}`, borderRadius:10, padding:"12px 16px", marginBottom:16, display:"flex", alignItems:"center", gap:12}}>
          <div style={{fontSize:22, flexShrink:0}}>{allOk ? <CheckCircleIcon size={24} color="#166534" /> : <RocketIcon size={24} color="#92400e" />}</div>
          <div style={{flex:1}}>
        ...
        {currentUser?.role === "admin" ? (
          <div style={{background:"#eff6ff",border:"1px solid #bfdbfe",borderRadius:8,padding:"10px 14px",marginBottom:16,fontSize:12,color:"#1e40af",lineHeight:1.8}}>
            <b style={{display:"flex",alignItems:"center",gap:4}}><UserIcon size={13} color="#1e40af" /> 管理者の設定内容</b><br />
          ...
          <div style={{background:"#f0fdf4",border:"1px solid #86efac",borderRadius:8,padding:"10px 14px",marginBottom:16,fontSize:12,color:"#166534",lineHeight:1.8}}>
            <b style={{display:"flex",alignItems:"center",gap:4}}><UserIcon size={13} color="#166534" /> メンバーの設定内容</b><br />
        ...
        {/* ── 全員共通：Gemini APIキー ── */}
        <div style={{fontSize:12,fontWeight:700,color:"#174f35",marginBottom:8,marginTop:4,display:"flex",alignItems:"center",gap:5}}><KeyIcon size={13} color="#174f35" /> AIアシスタント用 Gemini APIキー（各自が取得・入力）</div>
          ...
          <div style={{display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:4}}>
            ...
            <div style={{display:"flex", alignItems:"center", gap:8}}>
              {currentUser?.geminiConfigured
                ? <span style={{fontSize:10,background:"#d1fae5",color:"#059669",borderRadius:20,padding:"1px 8px",fontWeight:700,display:"flex",alignItems:"center",gap:3}}><CheckCircleIcon size={10} color="#059669" /> 設定済み</span>
                : <span style={{fontSize:10,background:"#fef3c7",color:"#d97706",borderRadius:20,padding:"1px 8px",fontWeight:700,display:"flex",alignItems:"center",gap:3}}><AlertIcon size={10} color="#d97706" /> 未設定</span>
              }
        ...
        <div style={{background:"#f0f5f2",borderRadius:8,padding:"14px 16px",fontSize:12,color:"#3d7a5e",lineHeight:2,marginBottom:24}}>
          <div style={{fontWeight:700,marginBottom:6,fontSize:13,display:"flex",alignItems:"center",gap:5}}><ClipboardIcon size={13} color="#3d7a5e" /> Gemini APIキー 取得手順</div>
        ...
        <div style={{borderTop:"1px solid #d8ede1",paddingTop:20,marginBottom:12}}>
          <div style={{fontSize:12,fontWeight:700,color:"#174f35",marginBottom:12,display:"flex",alignItems:"center",gap:5}}>
            <SendIcon size={13} color="#174f35" /> Gmail・GoogleタスクTODO 連携設定
          </div>
          ...
          {currentUser?.role === "admin" ? (
            <div>
              ...
              <div style={{background:"#f0f5f2",borderRadius:8,padding:"14px 16px",fontSize:12,color:"#3d7a5e",lineHeight:2,marginBottom:12}}>
                <div style={{fontWeight:700,marginBottom:6,fontSize:13,display:"flex",alignItems:"center",gap:5}}><ClipboardIcon size={13} color="#3d7a5e" /> Gmail OAuth Client ID 取得手順（管理者が実施）</div>
              ...
              <div style={{background:"#eff6ff",borderRadius:8,padding:"14px 16px",fontSize:12,color:"#1e40af",lineHeight:2,marginBottom:12,border:"1px solid #bfdbfe"}}>
                <div style={{fontWeight:700,marginBottom:6,fontSize:13,display:"flex",alignItems:"center",gap:5}}><CheckCircleIcon size={13} color="#1e40af" /> GoogleタスクTODO登録・カレンダー登録 — 追加設定（管理者が実施）</div>
                ...
                <div style={{marginTop:6,padding:"6px 10px",background:"#dbeafe",borderRadius:6,color:"#1e40af"}}>
                  <span style={{display:"flex",alignItems:"center",gap:4}}><LightbulbIcon size={11} color="#1e40af" /> 設定後は各メンバーが初回のみGoogleの認証ポップアップで「許可」を押すだけです。個別の追加設定は不要です。</span>
                </div>
              </div>
            </div>
          ) : (
            <div>
              {(() => {
                ...
                return sharedClientId ? (
                  <div style={{background:"#f0fdf4",border:"1px solid #86efac",borderRadius:8,padding:"12px 14px",marginBottom:16,fontSize:12,color:"#166534",display:"flex",alignItems:"center",gap:10}}>
                    <div style={{fontSize:20}}><CheckCircleIcon size={24} color="#166534" /></div>
                    <div>
                  ...
                  <div style={{background:"#fff7ed",border:"1px solid #fde68a",borderRadius:8,padding:"12px 14px",marginBottom:16,fontSize:12,color:"#92400e",display:"flex",alignItems:"center",gap:10}}>
                    <div style={{fontSize:20}}><AlertIcon size={24} color="#d97706" /></div>
                    <div>
              ...
              <div style={{background:"#f0fdf4",borderRadius:8,padding:"14px 16px",fontSize:12,color:"#166534",lineHeight:2,marginBottom:12,border:"1px solid #86efac"}}>
                <div style={{fontWeight:700,marginBottom:6,fontSize:13,display:"flex",alignItems:"center",gap:5}}><ClipboardIcon size={13} color="#166534" /> メンバーがやること（初回のみ）</div>
