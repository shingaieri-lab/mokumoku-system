...
export function AccountManager({ currentUser, onClose, inline, onUpdateProfile }) {
  ...
  const [form, setForm] = useState({ id:"", name:"", password:"", role:"member", color:PALETTE[0], email:"", signature:"", isStaff:false });
  ...
  const handleSubmit = () => {
    ...
    setEditingId(null); setShowAddForm(false); setErr("");
  };

  const startEdit = (a) => { setForm({ ...a, password: "" }); setEditingId(a.id); setErr(""); };
  const cancelEdit = () => { setForm({ id:"", name:"", password:"", role:"member", color:PALETTE[0], email:"", signature:"", isStaff:false }); setEditingId(null); setShowAddForm(false); setErr(""); };
