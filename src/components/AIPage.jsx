...
import { ACTION_TYPES, ACTION_RESULTS } from '../lib/constants.js';
import { addBusinessDays, clampToBusinessTime } from '../lib/date.js';
...
export function AIPage({ leads, onAdd, onUpdate, goLeads, goCalendar, aiConfig, currentUser, isMobile }) {
  ...
  const saveToLead=()=>{
    ...
    const nextDate=result.next_action_date_offset?addBusinessDays(actionDate,result.next_action_date_offset):"";
    if(savedActId){
  ...
  const saveAiGmailDraft = async () => {
    ...
    try {
      ...
      if (selLead && lead && result) {
        const followUpDate = result.next_action_date_offset ? addBusinessDays(actionDate, result.next_action_date_offset) : "";
        if (followUpDate) {
          onUpdate(lead.id, {
    ...
    <div style={{minHeight:"100vh",background:"#f0f5f2",fontFamily:"'Noto Sans JP','Hiragino Sans',sans-serif",color:"#174f35"}}>
      ...
      <div className="ai-layout" style={{display:"flex",height: isMobile ? "auto" : "calc(100vh - 77px)"}}>
        ...
        <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
          ...
          {result&&(
            <AIResultPanel
              result={result} selLead={selLead} lead={lead} saved={saved} actionDate={actionDate}
              saveToLead={saveToLead} editEmail={editEmail} setEditEmail={setEditEmail}
              copiedEmail={copiedEmail} setCopiedEmail={setCopiedEmail}
              aiGmailSaving={aiGmailSaving} aiGmailSaved={aiGmailSaved} saveAiGmailDraft={saveAiGmailDraft}
              aiCalSaving={aiCalSaving} aiCalSaved={aiCalSaved} saveAiCalTodo={saveAiCalTodo}
              goCalendar={goCalendar} goLeads={goLeads}
            />
          )}
        </div>
      </div>
