...
```
【セキュリティ対応報告】Vercelインシデント対応完了　新谷

■ インシデントの内容
2026年4月、Vercelの内部システムに不正アクセスが発生。原因はVercel自体ではなく、
第三者AIツール「Context.ai」のGoogle Workspace OAuthアプリが侵害されたことによるもの。
Vercelは「限定的な顧客に影響があった」と発表。
npmパッケージ・サプライチェーンへの影響はなし（複数社で確認済み）。

■ IOC確認
・Vercelダッシュボード → Integrations画面を確認
・不審なOAuthアプリ（Context.ai）なし。IOC該当なし。

■ 実施した対応
・KV_REST_API_TOKEN / KV_REST_API_READ_ONLY_TOKEN のローテーション
・KV_URL / REDIS_URL のローテーション
　（Vercel Storage → Settings → Rotate Secretsで一括対応・再デプロイ済み）
・ZOHO_WEBHOOK_TOKEN のローテーション・再デプロイ済み
　（ZohoCRM側のWebhookは未設定のためVercel側のみ対応）

■ セキュリティ強化（推奨対応）
・PASSWORD_ENC_KEY のローテーション
　組織標準（openssl rand -base64 32）に合わせて44文字Base64値に再発行。
　Sensitive設定済み（Production and Preview）。
・全変数にSensitive設定
　PASSWORD_ENC_KEYに設定完了。KV系4変数はUpstash Integration管理のため設定不可。
・MFA設定
　Passkey（Chrome on Windows）＋ Authenticator App（TOTP）の両方を設定。
　Two-Factor Authentication：Active。
・Vercelアクティビティログ確認
　全期間を確認。不審なアクセス・操作なし。

■ Upstash / Zohoアクセスログの確認
【Upstash】
　UsageログのCommandsグラフ・Keyspaceグラフを確認。
　SCAN系コマンドなし・TCP接続ゼロ・Keyspace安定。
　不審アクセスの痕跡なし。
【Zoho】
　アクセスログはCRM管理者権限が必要なため確認不可。
　管理者への確認依頼が必要。

■ openssl rand -base64 32 標準への対応
　PASSWORD_ENC_KEYを組織標準（openssl rand -base64 32）の
　44文字Base64形式に再発行済み。Sensitive設定維持（Production and Preview）。再デプロイ済み。

■ システム稼働状況
　本番環境（https://is-kanri.vercel.app）は稼働中。
　主に新谷が本番想定で試験的に活用しながら改修を行っている段階。
　正式な全社運用には至っていないため、外部顧客への直接影響は限定的。

■ 結果
　必須対応・推奨対応ともにすべて完了。
　IOC該当なし・不審アクセスの痕跡なし。
　引き続きZohoアクセスログの管理者確認を依頼予定。
```
