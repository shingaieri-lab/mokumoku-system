...
- 【Zoho行動同期の担当者設定】`push-action` はシステム共通のOAuthトークンで実行されるため、Zoho上の行動の担当者が常に管理者名になる。本来は記録したメンバーを担当者にしたい。解決には「本ツールのメンバー名 ↔ Zoho ユーザーID」の対応表が必要（Zoho `/users` APIで取得可能）。`field2` への `記録者: {名前}` 追記も一時的な回避策として検討したが、現時点では未実装のまま保留。
