...
// Zohoデータセンターに応じたドメイン（OAuth用）
function getZohoDomain(dc) {
  return dc === 'com' ? 'zoho.com' : 'zoho.jp';
}

// Zohoデータセンターに応じたAPIドメイン
function getZohoApiDomain(dc) {
  return dc === 'com' ? 'zohoapis.com' : 'zohoapis.jp';
}
...
async function zohoApi(method, path, body) {
  ...
  const apiDomain = getZohoApiDomain(cfg.dataCenter);
  const opts = {
    method,
    headers: {
  ...
  const r = await fetch(`https://www.${apiDomain}/crm/v2${path}`, opts);
  // 空レスポンス（204 No Content等）に対応するため text() で取得し、空ならステータスを含む情報を返す
  const text = await r.text();
  if (!text.trim()) {
    return { code: 'NO_DATA', message: `Empty response (HTTP ${r.status})`, status: r.status };
  }
  // JSON解析に失敗した場合は中身を返してデバッグに使えるようにする
  try {
    return JSON.parse(text);
  } catch (e) {
    return {
      code: 'NON_JSON_RESPONSE',
      message: `Non-JSON response (HTTP ${r.status}): ${text.slice(0, 200)}`,
      status: r.status,
    };
  }
}

module.exports = { getZohoDomain, getZohoApiDomain, getZohoTokens, saveZohoTokens, refreshZohoToken, zohoApi };
