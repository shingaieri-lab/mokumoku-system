# IS進捗管理ツール レビュー記録

レビュー開始：2026-03-23　／　最終更新：2026-05-13（セキュリティ修正・Zohoユーザーマッピング追加）

---

## このツールは何をするもの？

営業チームが「どの会社に連絡したか」「次はいつ連絡するか」を管理するツール。
AIが通話メモを読んで次の行動を提案したり、Googleカレンダーの空き時間を自動で探したりできる。

---

## ✅ 対応済み一覧

### 🔴 セキュリティ（P0）― 全件対応済み（2026-04-03）

| 問題 | 対応内容 |
|------|---------|
| ログイン情報がブラウザに平文保存されていた | HttpOnly Cookie に移行。localStorage から完全に除去 |
| CSRFトークン未実装 | SameSite=Strict Cookie に移行。追加対策不要な設計に変更 |
| Gemini APIキーがブラウザに露出していた | サーバー側専用エンドポイント（`/api/ai/analyze`）経由に変更 |
| Zoho の秘密情報がブラウザに渡っていた | サーバー側のみで保持。フロントには「設定済みか否か」のみ返す |
| Zoho Webhook に認証がなかった | トークンによるURL認証を追加（Zoho側のURL更新が別途必要） |
| postMessage の送信先が無制限だった | 自サイトのオリジンのみに限定 |
| リダイレクト先をユーザーが操作できた | サーバー側で自動生成に固定。設定画面は読み取り専用に変更 |
| 暗号化キーのフォールバック値が脆弱だった | 未設定時はサーバー起動を失敗させる設計に変更 |
| セッションの有効期限が30日と長すぎた | 7日に短縮 |

---

### 🔴 バグ・機能不全（P1）― 全件対応済み（2026-04-03）

| 問題 | 対応内容 |
|------|---------|
| KV障害時にレート制限が無効化されていた | 障害時は安全側（429エラー）を返すよう変更 |
| Zoho Webhook の同時処理でデータが消えることがあった | KVロックで排他制御を実装 |
| Zoho トークンの同時更新で競合が起きることがあった | KVロックで更新を1つに絞る設計に変更 |
| API保存失敗時に画面のデータが戻らなかった | 失敗時に保存前の状態に戻すロールバック処理を追加 |
| 招待コード発行でエラーが握りつぶされていた | エラー時にメッセージを表示するよう修正 |

---

### 🟠 品質改善（P2）― ページネーション以外は対応済み（2026-04-03）

| 問題 | 対応内容 |
|------|---------|
| パスワードが復号できる形でも保存されていた | bcrypt のみに統一。既存データも本番KVから削除済み |
| Google認証のエラー処理が4箇所にコピペされていた | 共通関数に集約 |
| エラーメッセージに技術用語（CORS等）が含まれていた | 一般ユーザー向けの文言に変更 |
| 外部祝日APIが失敗すると祝日が無視された | 既にフォールバック実装済みであることを確認 |
| クリック検知のコードに潜在的な不具合があった | 既に正しい実装であることを確認 |

---

### 🟢 UX・デザイン改善（2026-03-25 以前）

| 問題 | 対応内容 |
|------|---------|
| 候補日→メール画面でリード情報が引き継がれなかった | リード・テンプレートを自動引き継ぎ |
| AIページだけデザインが浮いていた（濃紺） | ライトグリーン系に統一 |
| ナビのラベルが小さすぎた | 10px に拡大 |
| 読み取り専用フィールドのコントラストが低かった | WCAG AA 準拠の色に変更 |
| テンプレートの選択状態が分かりにくかった | 選択中は背景・ボーダーで強調表示 |
| 初回セットアップが難しすぎた | 設定ウィザードを実装 |

---

### 🟢 バグ修正（2026-03-25）

| 問題 | 対応内容 |
|------|---------|
| パスワード再設定で競合が起きることがあった | エラー処理を強化 |
| Gmail OAuth トークンに有効期限がなかった | 55分キャッシュを実装 |
| コードの重複（Gmail下書き・祝日リスト・テンプレート初期値） | 共通化・一本化 |
| ストレージ上限超過時に何も起きなかった | アラート通知を追加 |

---

### 🟢 機能追加（2026-03-25 以降）

| 追加内容 |
|---------|
| Zoho CRM 連携（商談自動作成・重複防止・アクション履歴連携） |
| Zoho から住所・IS担当フラグを連携 |
| Zoho 手動取込 UI を追加 |
| KPI ダッシュボードに有効リード数・商談化率・月別合計を追加 |
| **Zoho ユーザーマッピング追加**（`feature/zoho-action-sync`、2026-05-13）：設定画面から Zoho ユーザー一覧を取得し、本ツールのアカウントと対応付け。行動同期時の「担当者」「割り当て先」にログインユーザーが反映される |

---

### 🟢 リファクタリング（2026-04-08〜15）

| 対応内容 |
|---------|
| **Vite移行・フル分割完了**（`feature/vite-migration`、PR #129、2026-04-14マージ）：CDN版ReactからVite+ESM構成へ全面移行。`src/App.jsx` をルートに、`src/pages/`・`src/components/actions/`・`src/components/leads/`・`src/components/nav/`・`src/components/settings/`・`src/components/ui/`・`src/components/wizard/`・`src/components/charts/` にページ・コンポーネントを階層分離。55モジュール、ビルドエラーゼロ |
| **server.js 分割完了**（`feature/file-split`）：983行のモノリスを `lib/`・`routes/` に8ファイル分割。`server.js` はエントリーポイント30行のみに |
| **コンポーネント分割完了**（`feature/component-split`）：`src/main.jsx` 4,885行 → 192行に削減。19コンポーネントを `src/components/` に、ビジネスロジックを `src/lib/` に分離。41モジュール、ビルドエラーゼロ |
| **API分離完了**（`feature/api-separation`）：7コンポーネントに直書きされていたfetch呼び出しを `src/lib/` に切り出し。`ai.js`, `account.js`, `zoho.js`, `authApi.js`, `gcal.js` に集約 |
| **デッドコード削除完了**（`chore/remove-dead-components`、PR #130、2026-04-14）：Vite移行後に残存していた旧フラット構造コンポーネント20本（5,295行）を削除。ビルドエラーゼロ確認済み |
| **ファイル分割フェーズ5完了**（`feature/file-split-phase5`、PR #140、2026-04-15）：`src/pages/` 内の300行超3ファイルを分割。CalendarSearchForm・CalendarSlotResults・AIInputPanel・EmailVariableForm を新規作成。300行超ファイルゼロを達成。71モジュール、ビルドエラーゼロ |

---

## ❌ 未対応・今後の課題

### 🔴 セキュリティ（2026-05-13 再監査で発覚）― 高優先度全件対応済み

| 優先度 | 問題 | 詳細 |
|--------|------|------|
| ~~高~~ | ~~**ログインエンドポイントにIPレート制限がない**~~ | ✅ 対応済み（`feature/security-review`、2026-05-13）：`/api/login` に IP ベースの `rateLimit` ミドルウェアを追加 |
| ~~高~~ | ~~**Zoho Client SecretがKVに平文保存**~~ | ✅ 対応済み（`feature/security-review`、2026-05-13）：保存時に AES-256-GCM で暗号化。`ENCRYPTION_KEY` 環境変数が必要 |
| ~~高~~ | ~~**APIキー（Gemini等）がKVに平文保存**~~ | ✅ 対応済み（`feature/security-review`、2026-05-13）：`ai_config.geminiKey` / `accounts[].geminiKey` を保存時に暗号化。未設定時は既存値を引き継ぎ |
| ~~中~~ | ~~**OAuth redirectURIが動的生成**~~ | ✅ 対応済み（`feature/security-review`、2026-05-13）：`ZOHO_REDIRECT_URI` 環境変数を優先使用。未設定時は従来の動的生成にフォールバック |
| ~~中~~ | ~~**入力バリデーションが薄い**~~ | ✅ 対応済み（`feature/input-validation`、2026-05-14）：`lib/validators.js` に Zod スキーマを定義。`/api/leads`・`/api/accounts`・`/api/master-settings`・`/api/email-tpls` に適用 |
| ~~中~~ | ~~**Webhook署名検証がない**~~ | ✅ 対応済み（`feature/input-validation`、2026-05-14）：`ZOHO_WEBHOOK_SECRET` 環境変数を追加。設定時は `X-Zoho-Webhook-Signature` ヘッダーの HMAC-SHA256 を検証。タイミング攻撃対策として `timingSafeEqual` を使用 |
| 低 | **セッション有効期限が7日** | 顧客情報を扱うツールとして、3日以下への短縮を検討 |



### 🔴 コード品質（CLAUDE.md違反）― 全件対応済み（2026-04-15）

#### ~~デッドコード残存~~ ✅ 対応済み（2026-04-14、PR #130）

旧フラット構造コンポーネント20本（5,295行）を削除済み。

#### ~~ファイル肥大化（300行超）~~ ✅ 再対応済み（2026-05-07、`feature/split-large-files`）

PR #140 で一度解消したが、その後の機能追加により再び肥大化。`feature/split-large-files` にて全件対応。

| ファイル | 対応内容 |
|------|------|
| `src/components/ui/Icons.jsx` | 579行 → 5行（バレルre-export）。用途別4ファイルに分割 |
| `src/components/actions/ActionHistoryPanel.jsx` | 391行 → 235行。DealActionBar・NextActionSection を切り出し |
| `src/pages/SettingsPage.jsx` | 321行 → 125行。PortalTab を切り出し |
| `src/pages/LeadsPage.jsx` | 314行 → 215行。LeadFilterBar を切り出し |
| `src/pages/AIPage.jsx` | 327行（findAvailableNextDate・clampToBusinessTime を lib/ai.js に移動済み） |

#### ~~API呼び出しのコンポーネント直書き~~ ✅ 再対応済み（2026-05-07、PR #238）

初回対応（2026-04-09）後、Zoho連携・カレンダー等の機能追加により11ファイル・18箇所に再発。
`lib/gcal.js` に `createMeetingEvent` / `createGoogleTask` / `fetchFreeBusyWithToken`、
`lib/ai.js` に `testGeminiKey` を追加し、全fetch直書きをゼロに。

---

### 🔴 コード品質（2026-05-01 監査で新規発覚）

| 問題 | 詳細 |
|------|------|
| ~~`date.js` と `holidays.js` で関数が二重定義~~ | ✅ 対応済み（2026-05-07、PR #239）：`date.js` から重複コードを全削除。`normalizeDate` のみ残し60行削減。`addBusinessDays`（UTC バグあり）も同時に除去 |
| ~~`toZenkaku` がコンポーネントにインライン定義~~ | ✅ 対応済み（2026-05-07、`feature/split-large-files`）：`lib/format.js` に切り出し。`buildDealShareText` も同ファイルに集約 |
| ~~`normDate` が `LeadsPage.jsx` にインライン定義~~ | ✅ 対応済み（2026-05-07、`feature/split-large-files`）：削除し `lib/date.js` の `normalizeDate` に統一 |

### 🟠 中優先度

| 問題 | 詳細 |
|------|------|
| ページネーション未実装 | 全リードをメモリに保持する設計。1000件超で遅延の恐れあり。現在100件未満のため当面は影響なし |
| ~~削除・編集ボタンが薄すぎる~~ | ✅ 対応済み（2026-04-15）: ボタン背景・ボーダー色を濃くして視認性を改善 |
| ~~成功メッセージが消えるのが速い~~ | ✅ 対応済み（2026-05-07、`feature/split-large-files`）：全メッセージを5秒以上に統一（コピー通知5秒・Zoho同期5秒・カレンダー登録6秒） |
| バルク操作の完了通知がない | インポート・削除後の件数通知がない。トースト通知の追加を検討 |
| ローディング表示が統一されていない | AI解析はスピナーあり、Gmail保存等はなし。統一を検討 |

### 🟡 低優先度（余裕があれば）

| 問題 | 詳細 |
|------|------|
| AI解析後の導線が不明 | 保存完了後に次のアクション（メール送信・候補日探し）へのボタンがない |
| 確認ダイアログがブラウザ標準のポップアップ | PortalTab の削除確認は2026-05-07にインライン確認ボタン方式に対応済み。ActionHistoryPanel・LeadMgmtTab・AccountManager は未対応 |
| モバイル対応が一部不完全 | AIページ・メールテンプレート・設定ページで固定幅が残っている |
| ボタンのタッチ領域が小さい | 高さ約20px。WCAG推奨の44px以上への対応を検討 |

---

## 👍 良い点

- KPIカードの視認性が高く、数値が一目でわかる
- リード検索がインクリメンタルサーチで使いやすい
- モバイル対応・ボトムナビが直感的
- エラーバウンダリが実装されており予期しないエラーに強い
- AI解析結果（興味度・トークポイント・次回アクション日）が実用的
- 候補日検索の設定が柔軟（曜日・バッファ・除外時間帯）
- Zoho CRM との双方向連携で営業フローの自動化が進んだ
