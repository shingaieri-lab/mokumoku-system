...
▼ 電話がつながった場合（取次・折電）
  → 資料送付が必要: 資料メールを送る＋2営業日後にフォロー電話（資料確認・内容説明）
  → 前向きな反応あり（興味度「高」）: 2〜3営業日後に商談日程を調整する（schedule）
  → 検討中・情報収集段階（興味度「中」）: 5営業日後にフォロー電話をかける
  → 反応薄い（興味度「低」）: 10営業日後に軽くフォロー電話をかける

▼ 電話がつながらない場合（不在・不通）
  ステップ1（初回）: next_action_typeは「call」（翌営業日に再電話する）。SMSは任意のため、必要と判断した場合のみnext_action_memoで言及する。
  ステップ2（2〜3回継続）: 意図確認メールを送る→3営業日後にフォロー電話をかける
  ステップ3（メール後も反応なし）: 5〜7営業日後に最終フォロー電話をかける
  ステップ4（長期音信不通）: 10〜15営業日後に低頻度フォロー、または一時クローズを検討する

▼ 追客タイミングの基準
  - 興味度「高」: 2〜3営業日以内に積極フォローする
  - 興味度「中」: 3〜5営業日後にフォローする
  - 興味度「低」: 5〜10営業日後に控えめにフォローする
